#torchrun --nproc_per_node 2 "/kuacc/users/bozyurt20/hpc_run/ready_scripts_nbs/script-prompting-llama13B-2nd prompting.py" --ckpt_dir /userfiles/bozyurt20/llama/llama_weights/13B --tokenizer_path /userfiles/bozyurt20/llama/llama_weights/tokenizer.model
    ...
    for item in dir_list_annotations:
        ...
        for line in annotations:
            ...
            for k in range(1, num_templates+1):
                ...
                prompt = llama_create_prompt_clipped(tokenizer, k, text, character, grammatical_number, max_no_tokens=2048)
                
                with torch.no_grad():
                    results = model.generate(
                                [prompt], max_gen_len=40, temperature=temperature, top_p=top_p
                            )
                out = results[0]
                
                print("One generation:", time.time() - start_time)
                
                out_stripped = out.replace(prompt + " ", "")
                x = out_stripped.find(".")
                y = out_stripped.find("\n")
                z = min(x,y)
                out_stripped = out_stripped[:z]
                
                match1, match2 = exactly_or_fuzzily_matched(out_stripped, character, gold_locations)
